...
- **Web screenshots:** After UI work, run `/dogfood-web` (skill: `.claude/skills/dogfood-web/`) — `pnpm test:e2e` then **read** `packages/web/test-results/screenshots/*.png`. Fail the bar if chrome, status, lists, or receipts show `ks_` / `ep_` / `ent_` / `principal_` or raw ISO instants, or if the page h1 does not match the active Capture / Recall / Inspect tab. After graph work, also read empty / fitted / selected-node map shots: `#knowledge-graph` must be `width/height: 100%` of `.graph-frame` (not a ~300×150 postage stamp), with labeled nodes and edges centered in the frame. Notes that use `works at` or `taught … in` must become nodes after Extract (a livesIn-only map is a P1). `pnpm test` does not run Playwright.
