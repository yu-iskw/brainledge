...
- 2026-08-19: Playwright `toHaveText(/PERSONAL/)` reads DOM, not CSS. `.shelf .meta { text-transform: uppercase }` still shouted Personal in screenshots. Assert `toHaveCSS('text-transform', 'none')` on `#overview-principal` and **read** the PNGs.
