...
1. Establish the surface's audience, job, product terminology, privacy boundaries, constraints, and incumbent visual truth.
...
5. Use the Playwright MCP plus the existing `packages/web` E2E suite for a bounded desktop+mobile inspection, fix findings as one batch, and perform at most one confirmation pass. Never claim a visual check that was not actually run.

## Brainledge-specific constraint

`packages/web` is currently a Vite web application without React. Do not introduce React, Next.js, Tailwind, shadcn/ui, or another framework/component system merely because an agent tool supports it. Framework adoption should be an explicit architecture decision based on the application architecture and design-system needs.
...
The web package already exposes build, unit-test, and Playwright E2E scripts. Continue to use the canonical repository commands documented in `AGENTS.md`; when UI behavior changes, include the relevant `packages/web` E2E coverage and rendered browser inspection.
