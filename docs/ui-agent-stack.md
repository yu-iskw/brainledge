...
This repository carries a small, auditable UI/UX agent stack for work in `packages/web`.
...
- **UI quality rule**: `.cursor/rules/ui-quality.mdc` applies to `packages/web/**/*` and requires a
...
It is not installed because Brainledge's current `packages/web` package uses Vite with TypeScript
and does not depend on React. A component framework should follow an explicit frontend architecture
decision rather than arrive as a side effect of agent tooling.
